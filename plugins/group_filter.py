...
async def next_page(bot, query):
    ...
    if SHORT_URL and SHORT_API:          
        if settings["button"]:
            btn = [[InlineKeyboardButton(text='{file.file_name}', return await query.answer("Entha mone", show_alert=True))] for file in files ]
        ...
        if settings["button"]:
            btn = [[InlineKeyboardButton(text=f"{file.file_name}", return await query.answer("Entha mone", show_alert=True))] for file in files ]
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb:
        ...
        cap = f'•𝗛𝗲𝘆 {message.from_user.mention}👋\n\n•📌𝗧𝗜𝗧𝗟𝗘: {search}\n•📣𝗬𝗢𝗨𝗥 𝗙𝗜𝗟𝗘 𝗜𝗦 𝗥𝗘𝗔𝗗𝗬 𝗡𝗢𝗪'
