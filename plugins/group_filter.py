...
async def next_page(bot, query):
    ...
    if SHORT_URL and SHORT_API:          
        if settings["button"]:
            btn = [[InlineKeyboardButton(text=f" {file.file_name}", url=await get_shortlink(f"https://telegram.dog/{temp.U_NAME}?start=files_{file.file_id}"))][InlineKeyboardButton(text=f"[{get_size(file.file_size)}] for file in files ]
        else:
